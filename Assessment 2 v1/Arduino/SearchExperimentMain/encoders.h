...
volatile long count_rightenc; // used by encoder to count the rotation
volatile byte state_rightenc; // used by encoder to store the state
volatile long count_leftenc; // used by encoder to count the rotation
volatile byte state_leftenc; // used by encoder to store the state
...
ISR( INT6_vect ) {
  ...
  // State:  new B   new A   old B   old A
    state_rightenc = state_rightenc | ( e0_B  << 3 );
    state_rightenc = state_rightenc | ( e0_A  << 2 );

    // print 'the left encoder is in state' and the state of the left encoder
    // Serial.print("The Right encoder is in state: ");
    ...
    // write an if statement to check if the state of the left encoder is 1 or 7 or 8 or 14 and up the count by 1
    //  and if the state of the left encoder is 2 or 4 or 11 or 13 then decrease the count by 1.

    if( state_rightenc == 1 || state_rightenc == 7 || state_rightenc == 8 || state_rightenc == 14 ) {
        count_rightenc--;
    }
     
     
    if( state_rightenc == 2 || state_rightenc == 4 || state_rightenc == 11 || state_rightenc == 13 ) {
        count_rightenc++;
    }
   


    // Serial.print("The count of the right encoder is: ");
    // Serial.println(count_rightenc);
    ...
    state_rightenc = state_rightenc >> 2;
...
ISR( PCINT0_vect ) {
    ...
    state_leftenc = state_leftenc | ( e1_B  << 3 );
    state_leftenc = state_leftenc | ( e1_A  << 2 );
    ...
    if( state_leftenc == 1 || state_leftenc == 7 || state_leftenc == 8 || state_leftenc == 14 ) {
        count_leftenc--;
    }
     
     
     if( state_leftenc == 2 || state_leftenc == 4 || state_leftenc == 11 || state_leftenc == 13 ) {
        count_leftenc++;
    }
    ...
    //
//     //
//     Serial.print(", The count of the left encoder is: ");
//    Serial.println(count_leftenc);

    // Shift the current readings (bits 3 and 2) down
    // into position 1 and 0 (to become prior readings)
    // This bumps bits 1 and 0 off to the right, "deleting"
    // them for the next ISR call. 
    state_leftenc = state_leftenc >> 2;
}
...
void setupEncoder0() 
{
    ...
    state_rightenc = state_rightenc | ( e0_B << 1 );
    state_rightenc = state_rightenc | ( e0_A << 0 );
...
void setupEncoder1() 
{
    ...
    // Shift values into correct place in state.
    // Bits 1 and 0  are prior states.
    state_leftenc = state_leftenc | ( e1_B << 1 );
    state_leftenc = state_leftenc | ( e1_A << 0 );
